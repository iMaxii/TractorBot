...
class UtilCog(commands.Cog):
    ...
    async def ping_command(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color=0x59575A, description=f"Pong! {ping} ms.")
    ...
    @commands.cooldown(1, 2, commands.BucketType.user)
    async def math_command(self, ctx: commands.Context, *, equation: str):
        try:
            result = eval(equation)
            embed = discord.Embed(color=0x59575A, description=f"{equation} = {result}")
            await ctx.send(embed=embed)
        except:
            embed = discord.Embed(
                description="¡Oops! No pude resolver esa operación.", color=0x925B5B
            )
            await ctx.send(embed=embed)
    ...
    async def snipe_message(self, ctx):
        ...
        if snipe is None:
            embed = discord.Embed(
                description="Parece que no hay nada que espíar.", color=0x925B5B
            )
            await ctx.send(embed=embed)
        else:
            snipe, _ = snipe

            embed = discord.Embed(color=0x59575A, description=snipe.content)
            embed.set_author(
    ...
    async def member_avatar(self, ctx: commands.Context, member: discord.Member = None):
        ...
        embed = discord.Embed(
            color=0x59575A, description=f"Este es el avatar de {user.mention}."
        )
